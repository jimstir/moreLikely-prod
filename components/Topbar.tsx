  ...
  const connectWallet = async () => {
    if (typeof window.ethereum !== 'undefined') {
      try {
        ...
        // Optionally POST to an endpoint to register the user wallet if it doesn't exist
        await fetch('/api/engagement', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          // A hack to just create the user profile via upsert using a view action on a dummy market
          body: JSON.stringify({ address: addr, marketId: 'CONNECT_INIT', action: 'view' })
        }).catch(console.error);
